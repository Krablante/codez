...
#[test]
fn test_deepseek_chat_wire_api_round_trips() {
    let provider_toml = r#"
name = "DeepSeek"
...
wire_api = "deepseek_chat"
    ...
    assert!(
        serialized.contains("wire_api = \"deepseek_chat\""),
        "serialized provider should keep the config value stable: {serialized}"
    );
}
...
fn test_built_in_model_providers_include_openrouter() {
    ...
    assert_eq!(
        providers
            ...
            .map(ModelProviderInfo::is_openrouter),
        Some(true)
    );
}

#[test]
fn test_openrouter_provider_retries_429() {
    let api_provider = ModelProviderInfo::create_openrouter_provider()
        .to_api_provider(None)
        .expect("OpenRouter provider should convert");
...
#[test]
fn test_openai_provider_does_not_retry_429_by_default() {
    let api_provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None)
        .to_api_provider(None)
        .expect("OpenAI provider should convert");
