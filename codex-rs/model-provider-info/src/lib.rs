...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// DeepSeek's OpenAI-compatible Chat Completions API at `/v1/chat/completions`.
    ///
    /// This is intentionally not the removed legacy `chat` wire API. Codex still
    /// runs its normal agent loop and maps DeepSeek chat chunks into Codex
    /// response events.
    ...
    pub fn is_openrouter(&self) -> bool {
        self.name == OPENROUTER_PROVIDER_NAME
            || self.base_url.as_deref().is_some_and(|base_url| {
                base_url.trim_end_matches('/') == OPENROUTER_DEFAULT_BASE_URL
            })
    }
...
pub fn built_in_model_providers(
    ...
    // Codez carries OpenRouter as a built-in provider so local-agent
    // integrations can rely on a stable provider id and env-key contract.
    [
