...
pub(crate) async fn run(
    ...
    let updated_input = results
        .iter()
        .filter_map(|result| result.data.updated_input.clone())
        .next_back();
            ...
            Some(0) => {
                ...
                if trimmed_stdout.is_empty() {
                    ...
                    if let Some(invalid_reason) = parsed.invalid_reason {
                        ...
                        let updated_input = parsed.updated_input;
                        ...
                        return dispatcher::ParsedHandler {
...
mod tests {
    ...
    #[test]
    fn updated_input_is_recorded() {
        let parsed = parse_completed(
            &handler(),
            run_result(
                Some(0),
                r#"{"hookSpecificOutput":{"hookEventName":"PreToolUse","updatedInput":{"command":"echo compact"}}}"#,
                "",
            ),
            Some("turn-1".to_string()),
        );
        ...
        assert_eq!(
            ...
            PreToolUseHandlerData {
                should_block: false,
                block_reason: None,
                updated_input: Some(serde_json::json!({ "command": "echo compact" })),
                additional_contexts_for_model: Vec::new(),
            }
        );
    ...
    #[test]
    fn legacy_block_still_blocks_with_updated_input() {
        let parsed = parse_completed(
            &handler(),
            run_result(
                Some(0),
                r#"{"decision":"block","reason":"do not run that","hookSpecificOutput":{"hookEventName":"PreToolUse","updatedInput":{"command":"echo compact"}}}"#,
                "",
            ),
            Some("turn-1".to_string()),
        );
