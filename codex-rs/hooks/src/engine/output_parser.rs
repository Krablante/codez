...
pub(crate) fn parse_pre_tool_use(stdout: &str) -> Option<PreToolUseOutput> {
    ...
    let updated_input = hook_specific_output.and_then(|output| output.updated_input.clone());
    let use_hook_specific_decision = hook_specific_output.is_some_and(|output| {
        output.permission_decision.is_some() || output.permission_decision_reason.is_some()
    });
    let invalid_reason = unsupported_pre_tool_use_universal(&universal)
        ...
        .or_else(|| unsupported_pre_tool_use_legacy_decision(decision.as_ref(), reason.as_deref()));
    ...
    Some(PreToolUseOutput {
        universal,
        block_reason,
        updated_input: if invalid_reason.is_none() {
            updated_input
        } else {
            None
        },
