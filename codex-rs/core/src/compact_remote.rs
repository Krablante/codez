...
use crate::session::turn::prune_prompt_history_for_sampling;
...
async fn run_remote_compact_task_inner_impl(
    ...
    turn_context: &Arc<TurnContext>,
    initial_context_injection: InitialContextInjection,
    prune_history_for_compaction: bool,
) -> CodexResult<()> {
    ...
    let mut new_history = loop {
        ...
        let prompt_input = if prune_history_for_compaction {
            prune_prompt_history_for_sampling(prompt_input)
        } else {
            prompt_input
        };
        let history_item_count = history.raw_items().len();
            ...
            .await
        {
            ...
            Err(err)
                if is_remote_compaction_context_window_error(&err) && history_item_count > 1 =>
            {
                error!(
                    turn_id = %turn_context.sub_id,
                    "Context window exceeded during remote compaction; removing oldest history item and retrying. Error: {err}"
                );
                history.remove_first_item();
                truncated_count += 1;
            }
            Err(err) => {
                if is_remote_compaction_context_window_error(&err) {
                    sess.set_total_tokens_full(turn_context.as_ref()).await;
                }
                log_remote_compact_attempt_failure(
                    sess.as_ref(),
                    turn_context.as_ref(),
                    &prompt,
                    &err,
    ...
    if truncated_count > 0 {
        info!(
            turn_id = %turn_context.sub_id,
            truncated_count,
            "trimmed oldest history items after remote compaction context window errors"
        );
    }
...
pub(crate) async fn process_compacted_history(
    ...
    // Mid-turn compaction and pre-turn context-limit compaction must inject initial context above
    // the last user message so the current turn's baseline remains explicit while the compaction
    // item stays last for model training.
...
async fn log_remote_compact_attempt_failure(
    sess: &Session,
    turn_context: &TurnContext,
    prompt: &Prompt,
    err: &CodexErr,
...
fn is_remote_compaction_context_window_error(err: &CodexErr) -> bool {
    match err {
        CodexErr::ContextWindowExceeded => true,
        CodexErr::InvalidRequest(message) => is_context_window_error_text(message),
        CodexErr::UnexpectedStatus(unexpected) => is_context_window_error_text(&unexpected.body),
        _ => false,
    }
}

fn is_context_window_error_text(message: &str) -> bool {
