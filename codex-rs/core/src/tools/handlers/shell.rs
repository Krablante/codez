...
use crate::tools::registry::pre_tool_use_updated_command;
use crate::tools::registry::replace_function_tool_argument;
...
fn updated_shell_argv(
    ...
    let command = pre_tool_use_updated_command(updated_input)?;
    Ok(invocation
        .session
        .user_shell()
        .derive_exec_args(&command, invocation.turn.tools_config.allow_login_shell))
}

fn apply_updated_shell_function_input(
    invocation: ToolInvocation,
    updated_input: JsonValue,
) -> Result<ToolInvocation, FunctionCallError> {
    let command = updated_shell_argv(&invocation, &updated_input)?;
    replace_function_tool_argument(invocation, "command", serde_json::json!(command))
}

fn apply_updated_local_shell_input(
    mut invocation: ToolInvocation,
    updated_input: JsonValue,
) -> Result<ToolInvocation, FunctionCallError> {
    let command = updated_shell_argv(&invocation, &updated_input)?;
    let ToolPayload::LocalShell { params } = &mut invocation.payload else {
        return Err(FunctionCallError::RespondToModel(
            "PreToolUse updatedInput can only rewrite local shell payloads".to_string(),
        ));
    };
