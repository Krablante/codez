...
#[tokio::test]
async fn exec_command_applies_pre_tool_use_updated_input_to_cmd() {
    let payload = ToolPayload::Function {
    ...
    let updated = handler
        .apply_pre_tool_use_updated_input(
            invocation,
            serde_json::json!({ "command": "echo compact" }),
        )
        .expect("updated input should apply");
    ...
    assert_eq!(value["cmd"], "echo compact");
    assert_eq!(value["workdir"], "subdir");
    assert_eq!(value["yield_time_ms"], 250);
}
