...
#[tokio::test]
async fn shell_command_applies_pre_tool_use_updated_input_to_command() {
    let payload = ToolPayload::Function {
        arguments: json!({
            "command": "printf verbose",
            "workdir": "subdir",
            "timeout_ms": 250
        })
        .to_string(),
    ...
    let updated = handler
        .apply_pre_tool_use_updated_input(invocation, json!({ "command": "printf compact" }))
    ...
    assert_eq!(value["command"], "printf compact");
