...
pub trait ToolHandler: Send + Sync {
    ...
    fn apply_pre_tool_use_updated_input(
        &self,
        _invocation: ToolInvocation,
        _updated_input: Value,
    ) -> Result<ToolInvocation, FunctionCallError> {
        Err(FunctionCallError::RespondToModel(format!(
            "PreToolUse hook returned updatedInput for unsupported tool {}",
...
pub(crate) fn pre_tool_use_updated_command(
    updated_input: &Value,
        ...
        Value::Object(object) => object
            ...
            .ok_or_else(|| {
                FunctionCallError::RespondToModel(
                    "PreToolUse hook returned updatedInput without a string command".to_string(),
                )
            }),
...
pub(crate) fn replace_function_tool_argument(
    ...
    let ToolPayload::Function { arguments } = &mut invocation.payload else {
        return Err(FunctionCallError::RespondToModel(
            "PreToolUse updatedInput can only rewrite function tool arguments".to_string(),
        ));
    };

    let mut value = serde_json::from_str::<Value>(arguments).map_err(|err| {
        FunctionCallError::RespondToModel(format!(
            "failed to parse function arguments before applying PreToolUse updatedInput: {err}"
        ))
    })?;
    let Value::Object(object) = &mut value else {
        return Err(FunctionCallError::RespondToModel(
            "PreToolUse updatedInput can only rewrite object function arguments".to_string(),
        ));
    };
    object.insert(field_name.to_string(), field_value);
    *arguments = serde_json::to_string(&value).map_err(|err| {
        FunctionCallError::RespondToModel(format!(
            "failed to serialize function arguments after applying PreToolUse updatedInput: {err}"
        ))
    })?;
    ...
    T: ToolHandler,
{
    ...
    fn apply_pre_tool_use_updated_input(
        ...
        ToolHandler::apply_pre_tool_use_updated_input(self, invocation, updated_input)
    }
...
impl ToolRegistry {
    ...
    pub(crate) async fn dispatch_any(
        ...
        if let Some(pre_tool_use_payload) = handler.pre_tool_use_payload(&invocation) {
            ...
            .await;
            if let Some(message) = pre_tool_use_outcome.block_message {
                let err = FunctionCallError::RespondToModel(message);
                dispatch_trace.record_failed(&err);
                return Err(err);
            }
            if let Some(updated_input) = pre_tool_use_outcome.updated_input {
                invocation = handler.apply_pre_tool_use_updated_input(invocation, updated_input)?;
                if !handler.matches_kind(&invocation.payload) {
                    let message =
                        format!("tool {display_name} produced incompatible updatedInput payload");
                    let err = FunctionCallError::Fatal(message);
                    dispatch_trace.record_failed(&err);
                    return Err(err);
                }
        ...
        let execution_payload_for_response = invocation.payload.clone();
        let execution_log_payload = execution_payload_for_response.log_payload();
        let (execution_mcp_server, execution_mcp_server_origin) = match &invocation.payload {
            ToolPayload::Mcp { server, .. } => {
                let manager = invocation
                    .session
                    .services
        ...
        let execution_mcp_server_ref = execution_mcp_server.as_deref();
        let execution_mcp_server_origin_ref = execution_mcp_server_origin.as_deref();
        ...
        let result = otel
            .log_tool_result_with_tags(
                &display_name,
                &call_id_owned,
                execution_log_payload.as_ref(),
                &metric_tags,
                execution_mcp_server_ref,
                execution_mcp_server_origin_ref,
                || {
