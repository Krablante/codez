...
///
/// Manual compaction and pre-turn model-downshift compaction use `DoNotInject`: they replace
/// history with a summary and clear `reference_context_item`, so the next regular turn will fully
/// reinject initial context after compaction.
///
/// Mid-turn compaction and pre-turn context-limit compaction use `BeforeLastUserMessage` because
/// the current turn's context must remain explicit after compaction; we therefore inject initial
/// context into the replacement history just above the last real user message.
...
async fn run_compact_task_inner_impl(
    ...
    let preserved_tail_user_items = if matches!(
        initial_context_injection,
        InitialContextInjection::BeforeLastUserMessage
    ) {
        last_real_user_response_item(history_items)
            .into_iter()
            .inspect(|_| {
                let _ = user_messages.pop();
            })
            .collect::<Vec<_>>()
    } else {
        Vec::new()
    ...
    let mut new_history = build_compacted_history_with_preserved_tail(
        Vec::new(),
        &user_messages,
        &preserved_tail_user_items,
        &summary_text,
...
fn last_real_user_response_item(items: &[ResponseItem]) -> Option<ResponseItem> {
    items.iter().rev().find_map(|item| {
        let Some(TurnItem::UserMessage(user)) = crate::event_mapping::parse_turn_item(item) else {
            return None;
        };
        (!is_summary_message(&user.message())).then(|| item.clone())
    })
}
...
pub(crate) fn build_compacted_history(
    ...
    build_compacted_history_with_preserved_tail(
        initial_context,
        user_messages,
        &[],
        summary_text,
...
fn build_compacted_history_with_preserved_tail(
    mut history: Vec<ResponseItem>,
    user_messages: &[String],
    preserved_tail_user_items: &[ResponseItem],
    summary_text: &str,
    max_tokens: usize,
) -> Vec<ResponseItem> {
    ...
    history.extend_from_slice(preserved_tail_user_items);
