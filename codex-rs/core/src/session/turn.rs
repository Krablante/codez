...
pub(crate) fn prune_prompt_history_for_sampling(items: Vec<ResponseItem>) -> Vec<ResponseItem> {
    ...
    let preserved_image_call_ids = items
        ...
        .filter_map(image_output_call_id)
        .collect::<HashSet<_>>();
    ...
    items
        ...
        .enumerate()
        .filter_map(|(idx, item)| {
            if idx < active_turn_start && is_stale_context_prunable_history_item(&item) {
                return None;
            }
            if idx >= active_turn_start
                || !is_prompt_prunable_history_item(&item)
                || preserves_image_output_context(&item, &preserved_image_call_ids)
            {
                Some(item)
            } else {
                None
...
fn is_active_turn_context_item(item: &ResponseItem) -> bool {
    match item {
        ResponseItem::Message { role, content, .. } if role == "developer" => {
            is_contextual_dev_message_content(content)
        }
...
fn is_prompt_prunable_history_item(item: &ResponseItem) -> bool {
    matches!(
        item,
        ...
        ResponseItem::FunctionCallOutput { output, .. }
            ...
            if !output_contains_input_image(output)
    )
}

fn output_contains_input_image(output: &FunctionCallOutputPayload) -> bool {
...
fn image_output_call_id(item: &ResponseItem) -> Option<String> {
    match item {
        ResponseItem::FunctionCallOutput { call_id, output }
        | ResponseItem::CustomToolCallOutput {
            call_id, output, ..
        } if output_contains_input_image(output) => Some(call_id.clone()),
        _ => None,
    }
}

fn preserves_image_output_context(
    item: &ResponseItem,
    preserved_call_ids: &HashSet<String>,
) -> bool {
    match item {
        ResponseItem::FunctionCall { call_id, .. }
        | ResponseItem::CustomToolCall { call_id, .. } => preserved_call_ids.contains(call_id),
        _ => false,
    }
}
...
#[cfg(test)]
#[path = "turn_prompt_pruning_tests.rs"]
mod turn_prompt_pruning_tests;
