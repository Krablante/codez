...
async fn pre_tool_use_rewrites_shell_command_before_execution() -> Result<()> {
    ...
    let marker = std::env::temp_dir().join("pretooluse-shell-command-rewrite-marker");
    let original_command = format!("printf original > {}", marker.display());
    let rewritten_command = format!("printf rewritten > {}", marker.display());
    ...
    let rewritten_for_hook = rewritten_command.clone();
    let mut builder = test_codex()
        .with_pre_build_hook(move |home| {
            if let Err(error) =
                write_pre_tool_use_hook(home, Some("^Bash$"), "rewrite", &rewritten_for_hook)
            {
                panic!("failed to write pre tool use hook test fixture: {error}");
            }
        })
    ...
    if marker.exists() {
        fs::remove_file(&marker).context("remove leftover pre tool use rewrite marker")?;
    }
    ...
    requests[1].function_call_output(call_id);
    assert_eq!(
        fs::read_to_string(&marker).context("read pre tool use rewrite marker")?,
        "rewritten"
    );
...
async fn pre_tool_use_rewrites_exec_command_before_execution() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let call_id = "pretooluse-exec-command-rewrite";
    let marker = std::env::temp_dir().join("pretooluse-exec-command-rewrite-marker");
    let original_command = format!("printf original > {}", marker.display());
    let rewritten_command = format!("printf rewritten > {}", marker.display());
    ...
    let responses = mount_sse_sequence(
        ...
        vec![
            ...
            sse(vec![
                ev_response_created("resp-2"),
                ev_assistant_message("msg-1", "exec command rewrote it"),
                ev_completed("resp-2"),
            ]),
        ],
    )
    ...
    if marker.exists() {
        fs::remove_file(&marker).context("remove leftover exec rewrite marker")?;
    }

    test.submit_turn("run the rewritten exec command").await?;

    let requests = responses.requests();
    assert_eq!(requests.len(), 2);
    requests[1].function_call_output(call_id);
    assert_eq!(
        fs::read_to_string(&marker).context("read pre tool use exec rewrite marker")?,
        "rewritten"
    );
