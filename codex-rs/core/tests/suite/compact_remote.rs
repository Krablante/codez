...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        ...
        "After five varied API-key-auth turns, remote manual compaction omits service_tier, reuses prompt_cache_key, carries accumulated prompt history, and still omits responses-only fields.",
    )
    .await?;
...
async fn remote_manual_compact_chatgpt_auth_reuses_service_tier_and_prompt_cache_key() -> Result<()>
{
    ...
    assert_remote_manual_compact_request_parity(
        ...
        "After five varied ChatGPT-auth turns, remote manual compaction reuses service_tier and prompt_cache_key, carries accumulated prompt history, and omits responses-only fields.",
    )
    .await?;
...
async fn auto_remote_compact_trims_function_call_history_to_fit_context_window() -> Result<()> {
    ...
    assert!(
        !compact_request.has_function_call(retained_call_id)
            && compact_request
                .function_call_output_text(retained_call_id)
                .is_none(),
        "expected auto compact request to prune older function call/result history before compaction"
    );
    assert!(
        !compact_request.has_function_call(trimmed_call_id)
            && compact_request
    ...
    assert_eq!(
        compact_request.inputs_of_type("function_call").len(),
        0,
        "expected no function calls after auto-compact pruning and boundary trimming"
    );
    assert_eq!(
        compact_request.inputs_of_type("function_call_output").len(),
        0,
        "expected no function call outputs after auto-compact pruning and boundary trimming"
    );
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    baseline_codex
        ...
        .await?;
    wait_for_turn_complete(&baseline_codex).await;
    ...
    override_codex
        ...
        .await?;
    wait_for_turn_complete(&override_codex).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    wait_for_event_with_timeout(
        &initial.codex,
        |ev| matches!(ev, EventMsg::ShutdownComplete),
        REMOTE_COMPACT_TURN_COMPLETE_TIMEOUT,
    )
    .await;
...
-> Result<()> {
    ...
    insta::assert_snapshot!(
        ...
        format_labeled_requests_snapshot(
            "Remote mid-turn continuation compaction after realtime was closed before the turn: the initial second-turn request emits realtime-end instructions, compact input prunes sampling-only context, and the continuation request does not restate realtime-end after compaction because the current turn already established the inactive baseline.",
            &[
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn snapshot_request_shape_remote_compact_resume_prunes_stale_realtime_end() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert!(
        !after_resume_request
            .body_json()
            .to_string()
            .contains("<realtime_conversation>"),
        "post-resume sampling should prune stale realtime-end context after compaction"
    );

    insta::assert_snapshot!(
        "remote_compact_resume_prunes_stale_realtime_end_shapes",
        format_labeled_requests_snapshot(
            "After remote manual /compact and resume, the first resumed turn rebuilds history from the compaction item and prunes stale realtime-end context from reconstructed previous-turn settings.",
            &[
                ("Remote Compaction Request", &compact_request),
...
-> Result<()> {
    ...
    insta::assert_snapshot!(
        ...
        format_labeled_requests_snapshot(
            "Remote pre-turn auto-compaction prunes sampling-only context from compact input, then re-injects the context override for the post-compaction request while excluding the incoming user message.",
            &[
                ("Remote Compaction Request", &compact_request),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn remote_pre_turn_compaction_retries_after_context_window_exceeded() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            ...
            responses::sse(vec![
                responses::ev_assistant_message("m3", "REMOTE_POST_COMPACT_FINAL_REPLY"),
                responses::ev_completed_with_tokens("r3", /*total_tokens*/ 80),
            ]),
        ],
    )
    ...
    assert_eq!(
        requests.len(),
        3,
        "expected the turn to continue after remote compaction retries"
    );

    let compact_requests = compact_mock.requests();
    let first_compact_request = &compact_requests[0];
    let second_compact_request = &compact_requests[1];
    assert!(
        estimate_compact_payload_tokens(second_compact_request)
            < estimate_compact_payload_tokens(first_compact_request),
        "expected retry compact request to shrink after dropping the oldest history"
    );
    let first_compact_body = first_compact_request.body_json();
    let first_input = first_compact_body["input"]
        .as_array()
        .unwrap_or_else(|| panic!("first compact attempt missing input array"));
    let retry_compact_body = second_compact_request.body_json();
    let retry_input = retry_compact_body["input"]
        .as_array()
        .unwrap_or_else(|| panic!("retry compact attempt missing input array"));
    assert_eq!(
        retry_input.len(),
        first_input.len().saturating_sub(1),
        "retry should drop exactly one oldest compact input item"
    );
    if let (Some(first_before), Some(first_after)) = (first_input.first(), retry_input.first()) {
        assert_ne!(
            first_before, first_after,
            "expected the retry compact attempt to drop the oldest turn"
        );
    } else {
