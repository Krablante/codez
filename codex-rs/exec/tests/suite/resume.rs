...
fn write_dummy_chatgpt_auth(codex_home: &Path) -> anyhow::Result<()> {
    let fake_jwt = "eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0..c2ln";
    std::fs::write(
...
fn write_tiny_png(path: &Path) -> anyhow::Result<()> {
    let image_bytes: &[u8] = &[
...
async fn exec_resume_prunes_old_tool_history_from_model_prompt() -> anyhow::Result<()> {
    let test = test_codex_exec();
    let server = responses::start_mock_server().await;
    let repo_root = exec_repo_root()?;

    let old_call_id = "old-shell-call";
    responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_response_created("resp-old-tool"),
            responses::ev_shell_command_call(old_call_id, "printf old-tool-output"),
            responses::ev_completed("resp-old-tool"),
        ]),
    )
    .await;
    let first_follow_up = responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-old-final", "old final"),
            responses::ev_completed("resp-old-final"),
        ]),
    )
    .await;

    let marker = format!("resume-prune-{}", Uuid::new_v4());
    ...
    assert!(
        first_follow_up
            .single_request()
            .function_call_output_text(old_call_id)
            .is_some(),
        "same-turn tool output should still be sent before it becomes old history"
    );
    ...
    let resumed_request = responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-resumed-final", "resumed final"),
            responses::ev_completed("resp-resumed-final"),
        ]),
    )
    .await;
    let resumed_marker = format!("resume-prune-2-{}", Uuid::new_v4());
    test.cmd_with_server(&server)
        .arg("--skip-git-repo-check")
        .arg("--dangerously-bypass-approvals-and-sandbox")
        .arg("-C")
    ...
    assert!(
        !request.has_function_call(old_call_id)
            && request.function_call_output_text(old_call_id).is_none(),
        "codex exec resume should prune old model-visible tool call/output history"
    );
    assert!(
        request.body_contains_text(&marker) && request.body_contains_text(&resumed_marker),
        "resume request should keep durable user messages while pruning tool artifacts"
    );
...
#[cfg(not(target_os = "windows"))]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_resume_keeps_image_bearing_tool_output_context() -> anyhow::Result<()> {
    let test = test_codex_exec();
    let server = responses::start_mock_server().await;
    let image_path = test.cwd_path().join("resume-prune-image.png");
    write_tiny_png(&image_path)?;

    let image_call_id = "old-view-image-call";
    let view_image_args = json!({ "path": "resume-prune-image.png" }).to_string();
    responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_response_created("resp-image-tool"),
            responses::ev_function_call(image_call_id, "view_image", &view_image_args),
            responses::ev_completed("resp-image-tool"),
        ]),
    )
    .await;
    let first_follow_up = responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-image-final", "image final"),
            responses::ev_completed("resp-image-final"),
        ]),
    )
    .await;

    let marker = format!("resume-image-prune-{}", Uuid::new_v4());
    test.cmd_with_server(&server)
        .arg("--skip-git-repo-check")
        .arg("--dangerously-bypass-approvals-and-sandbox")
        .arg("-C")
    ...
    let first_output = first_follow_up
        .single_request()
        .function_call_output(image_call_id);
    assert!(
        first_output
            .get("output")
        ...
        "view_image should produce an image-bearing function_call_output"
    );
    ...
    let resumed_request = responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-image-resumed", "image resumed"),
            responses::ev_completed("resp-image-resumed"),
        ]),
    )
    .await;
    test.cmd_with_server(&server)
        ...
        .arg("resume")
        .arg(&session_id)
        .arg(format!("resume-image-prune-2-{}", Uuid::new_v4()))
        .assert()
        .success();
    ...
    assert!(
        request.has_function_call(image_call_id)
            && image_output
                .get("output")
                .and_then(Value::as_array)
                .is_some_and(|items| {
        ...
        "codex exec resume should preserve old image-bearing tool call/output context"
    );
...
#[cfg(not(target_os = "windows"))]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_resume_remote_auto_compact_prunes_old_tool_history() -> anyhow::Result<()> {
    let test = test_codex_exec();
    write_dummy_chatgpt_auth(test.home_path())?;
    let server = responses::start_mock_server().await;
    let repo_root = exec_repo_root()?;

    let old_call_id = "compact-old-shell-call";
    responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_response_created("resp-compact-tool"),
            responses::ev_shell_command_call(old_call_id, "printf compact-old-tool-output"),
            responses::ev_completed("resp-compact-tool"),
        ]),
    )
    .await;
    responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-compact-old-final", "compact old final"),
            responses::ev_completed("resp-compact-old-final"),
        ]),
    )
    .await;

    let marker = format!("resume-remote-compact-{}", Uuid::new_v4());
    test.cmd_with_server(&server)
        ...
        .arg(&repo_root)
        .arg("-c")
        .arg("model_auto_compact_token_limit=120")
        .arg(&marker)
        .assert()
        .success();
    ...
    responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-compact-second-final", "compact second final"),
            responses::ev_completed_with_tokens("resp-compact-second-final", 500_000),
        ]),
    )
    .await;
    let second_marker = format!("resume-remote-compact-2-{}", Uuid::new_v4());
    test.cmd_with_server(&server)
        .env_remove("OPENAI_API_KEY")
        .arg("--skip-git-repo-check")
        .arg("--dangerously-bypass-approvals-and-sandbox")
    ...
    let compact_request = responses::mount_compact_user_history_with_summary_once(
        &server,
        "EXEC_REMOTE_COMPACT_SUMMARY",
    )
    .await;
    let post_compact_request = responses::mount_sse_once(
        &server,
        responses::sse(vec![
            responses::ev_assistant_message("msg-post-compact", "post compact final"),
            responses::ev_completed("resp-post-compact"),
        ]),
    )
    .await;
    let resumed_marker = format!("resume-remote-compact-3-{}", Uuid::new_v4());
    test.cmd_with_server(&server)
        .env_remove("OPENAI_API_KEY")
        .arg("--skip-git-repo-check")
        .arg("--dangerously-bypass-approvals-and-sandbox")
    ...
    let compact = compact_request.single_request();
    assert_eq!(compact.path(), "/v1/responses/compact");
    assert!(
        !compact.has_function_call(old_call_id)
            && compact.function_call_output_text(old_call_id).is_none(),
        "remote compact request from codex exec resume should prune old tool call/output history"
    );
    assert!(
        post_compact_request
            .single_request()
            .body_contains_text("EXEC_REMOTE_COMPACT_SUMMARY"),
        "post-compaction sampling request should use remote compact replacement history"
    );
