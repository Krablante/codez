...
#[test]
fn external_cli_provider_skips_managed_requirements() {
    let overrides = codex_utils_cli::CliConfigOverrides {
        raw_overrides: vec![
            "model_provider=\"deepseek\"".to_string(),
            "model_providers.deepseek={ name=\"DeepSeek\", base_url=\"https://api.deepseek.com/v1\", env_key=\"DEEPSEEK_API_KEY\", wire_api=\"deepseek_chat\", requires_openai_auth=false }".to_string(),
        ],
    };

    assert!(should_ignore_managed_requirements_for_cli_provider(
        &overrides
    ));
}

#[test]
fn openai_cli_provider_keeps_managed_requirements() {
    let overrides = codex_utils_cli::CliConfigOverrides {
        raw_overrides: vec![
            "model_provider=\"openai\"".to_string(),
            "model_providers.openai={ name=\"OpenAI\", base_url=\"https://api.openai.com/v1\", env_key=\"OPENAI_API_KEY\", wire_api=\"responses\", requires_openai_auth=true }".to_string(),
        ],
    };
