...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    #[allow(clippy::print_stderr)]
    let ignore_managed_requirements =
        should_ignore_managed_requirements_for_cli_provider(&config_overrides);
...
fn should_ignore_managed_requirements_for_cli_provider(
    cli_kv_overrides: &codex_utils_cli::CliConfigOverrides,
) -> bool {
    let Ok(overrides) = cli_kv_overrides.parse_overrides() else {
        return false;
    };

    let Some(provider_id) = overrides.iter().find_map(|(key, value)| {
        (key == "model_provider")
    ...
    let provider_table_key = format!("model_providers.{provider_id}");
    let provider_requires_auth_key = format!("model_providers.{provider_id}.requires_openai_auth");

    overrides.iter().any(|(key, value)| {
        if key == &provider_requires_auth_key {
            return value.as_bool() == Some(false);
        }
        if key != &provider_table_key {
            return false;
        }
        value
            .as_table()
