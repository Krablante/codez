...
const RESUME_ROLLOUT_READ_CHUNK_BYTES: u64 = 1024 * 1024;
...
fn parse_rollout_item_line(line: &str, parse_errors: &mut usize) -> Option<RolloutItem> {
    ...
    match serde_json::from_value::<RolloutLine>(value) {
        Ok(rollout_line) => Some(rollout_line.item),
        Err(e) => {
...
impl RolloutRecorder {
    ...
    pub async fn load_rollout_items(
        ...
        for line in text.lines() {
            if let Some(item) = parse_rollout_item_line(line, &mut parse_errors) {
    ...
    pub async fn load_rollout_items_for_resume(
        ...
        match Self::load_rollout_resume_slice(path).await? {
            Some((items, parse_errors)) => {
                let thread_id = thread_id_from_items(&items);
                tracing::debug!(
                    "Loaded compact resume slice with {} items, thread ID: {:?}, parse errors: {}",
                    items.len(),
                    thread_id,
                    parse_errors,
                );
            ...
            None => Self::load_rollout_items(path).await,
        }
    }

    async fn load_rollout_resume_slice(
        path: &Path,
    ) -> std::io::Result<Option<(Vec<RolloutItem>, usize)>> {
        let session_meta = match read_session_meta_line(path).await {
            Ok(meta_line) => meta_line,
            Err(err) => {
                trace!("failed to read rollout session metadata for compact resume slice: {err}");
                return Ok(None);
            }
        };
        ...
        let mut tail_items_reversed: Vec<RolloutItem> = Vec::new();
        let mut parse_errors = 0usize;

        while position > 0 {
            let chunk_start = position.saturating_sub(RESUME_ROLLOUT_READ_CHUNK_BYTES);
            let bytes_to_read = usize::try_from(position - chunk_start)
                .map_err(|_| IoError::other("rollout read chunk is too large"))?;
            let mut buffer = vec![0u8; bytes_to_read];
            file.seek(SeekFrom::Start(chunk_start)).await?;
            file.read_exact(&mut buffer).await?;
            let mut combined = buffer;
            combined.extend_from_slice(&partial_line);
            let lines_text = if chunk_start > 0 {
                let Some(first_newline) = combined.iter().position(|byte| *byte == b'\n') else {
                    partial_line = combined;
                    position = chunk_start;
                    continue;
                };
                partial_line = combined[..first_newline].to_vec();
                &combined[first_newline + 1..]
            } else {
                combined.as_slice()
            };

            for line in lines_text.split(|byte| *byte == b'\n').rev() {
                let line = match std::str::from_utf8(line) {
                    Ok(line) => line,
                    Err(e) => {
                        warn!("failed to parse rollout line as UTF-8: {e}");
                        parse_errors = parse_errors.saturating_add(1);
                        continue;
                    }
                };
                let Some(item) = parse_rollout_item_line(line, &mut parse_errors) else {
                    continue;
                };
                match item {
                        ...
                        if compacted.replacement_history.is_some() =>
                    {
                        ...
                        items.extend(tail_items_reversed.into_iter().rev());
                        return Ok(Some((items, parse_errors)));
                    }
                    item => tail_items_reversed.push(item),
                }
            }
