...
mod tests {
    ...
    fn session_source_from_startup_arg_normalizes_custom_values() {
        assert_eq!(
            SessionSource::from_startup_arg("codez").unwrap(),
            SessionSource::Custom("codez".to_string())
        );
        assert_eq!(
            SessionSource::from_startup_arg(" Codez ").unwrap(),
            SessionSource::Custom("codez".to_string())
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("CODEZ".to_string()).restriction_product(),
            Some(Product::Codez)
        );
        assert_eq!(
        ...
        assert_eq!(
            SessionSource::Custom("codez-dev".to_string()).restriction_product(),
            None
        );
    }
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("codez-dev".to_string())
                .matches_product_restriction(&[Product::Codez])
        );
        assert!(SessionSource::Custom("codez-dev".to_string()).matches_product_restriction(&[]));
    }
