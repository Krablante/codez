...
impl<'de> Deserialize<'de> for ReasoningItemReasoningSummary {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        match TaggedSummary::deserialize(value).map_err(serde::de::Error::custom)? {
            TaggedSummary::SummaryText { text } => Ok(Self::SummaryText { text }),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn reasoning_summary_accepts_openai_tagged_shape() -> Result<()> {
        let value: ReasoningItemReasoningSummary = serde_json::from_value(serde_json::json!({
            "type": "summary_text",
            "text": "checked files"
        }))?;
    ...
    #[test]
    fn reasoning_summary_accepts_openrouter_string_shape() -> Result<()> {
        let value: ReasoningItemReasoningSummary =
            serde_json::from_value(serde_json::json!("checked files"))?;
