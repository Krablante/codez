    ...
    match event.kind.as_str() {
        ...
        "response.failed" => {
            if let Some(resp_val) = event.response {
                return Err(ResponsesEventError::Api(api_error_from_error_value(
                    resp_val.get("error"),
...
fn api_error_from_error_value(error: Option<&Value>, default_message: &str) -> ApiError {
    let Some(error_value) = error else {
        return ApiError::Stream(default_message.to_string());
    };
    match serde_json::from_value::<Error>(error_value.clone()) {
        Ok(error) => api_error_from_parsed_error(error, default_message),
        Err(_) => ApiError::Retryable {
            message: error_value
                .get("message")
                .and_then(Value::as_str)
...
fn api_error_from_parsed_error(error: Error, default_message: &str) -> ApiError {
    if is_context_window_error(&error) {
        ...
        let message = error.message.unwrap_or_else(|| default_message.to_string());
...
mod tests {
    ...
    #[tokio::test]
    async fn parses_openrouter_reasoning_delta() {
        let events = run_sse(vec![
            json!({
    ...
    #[tokio::test]
    async fn response_error_event_is_reported_without_waiting_for_stream_close() {
        let raw_error = r#"{"type":"response.error","error":{"code":"rate_limit_exceeded","message":"OpenRouter rate limit. Please try again in 2s."}}"#;
        ...
        match &events[0] {
            Err(ApiError::Retryable { message, delay }) => {
                assert_eq!(message, "OpenRouter rate limit. Please try again in 2s.");
    ...
    #[tokio::test]
    async fn plain_error_event_is_reported_without_waiting_for_stream_close() {
        let raw_error = r#"{"type":"error","error":{"message":"Provider stream failed"}}"#;
        ...
        match &events[0] {
            Err(ApiError::Retryable { message, delay }) => {
                assert_eq!(message, "Provider stream failed");
